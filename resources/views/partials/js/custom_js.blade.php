...
//        Update Values
    ...
    function clearForm(form){
        ...
        // let option-ut = document.getElementById("option-ut");
        // let doe = document.getElementById("doe");
    $(window).on('load', function() {
        $("#usernamedivteacher").hide();
        $("#doeinput").attr('required', true);
    });

    function doe() {
        if($("#user_type option:selected").val() == 'EOrg2eXxeayB3Q'){
            $("#doe").show();
            $("#usernamedivteacher").hide();
            $("#star").hide();
            $("#doeinput").attr('required', true);
            ...
            $("#doeinput").attr('required', false);
            $("#usernameinput").attr('required', true);
            $("#doe").hide();
            $("#usernamedivteacher").show();
            $("#star").show();
        }
    }
    ...
    const data = {
        ...
        datasets: [{
            data: [
                @foreach (Qs::getAllUserTypes() as $userType)
                    {{ \App\Repositories\UserRepo::getAll()->where('user_type', $userType)->count() }},
                @endforeach
            ],
            backgroundColor: ['rgb(255, 99, 132)', 'rgb(132, 99, 255)', 'rgb(99, 255, 132)', 'rgb(0, 99, 132)', 'rgb(255, 99, 0)'],
            // borderWidth: 1,
            // borderColor: 'rgb(255)',
            hoverBorderWidth: 3,
