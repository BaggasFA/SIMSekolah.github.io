...
class StudentRepo {
    ...
    public function findStudentsByClass($class_id, $session)
    {
        return $this->activeStudents()->where(['my_class_id' => $class_id, 'session' => $session])->with(['my_class', 'user'])->get()->sortBy('user.name');
    }
    ...
    public function getRecordMark(array $data)
    {
        // dd($data);
        return $this->activeStudents()->where($data)->with('user');
    }
